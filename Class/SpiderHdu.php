...
class SpiderHdu extends SpiderBasic
{
    ...
    function submitJob()
    {
        ...
        $curl->setHeader("Referer: http://acm.hdu.edu.cn/submit.php?pid=$problem_id");
        ...
        $request=array(
            ...
            "usercode"=>$this->job_info['source_code'],
            "check"=>0
        );
        $return=$curl->post("http://acm.hdu.edu.cn/submit.php?action=submit",$request,10);

        if($curl->getResponseCode()=="302" && empty($return))//正常情况无返回（跳转）
        {
            $html=$curl->get("http://acm.hdu.edu.cn/status.php?user=$username",10);
            if(strpos($html,'Register new ID')!==false)
            {
                $this->is_login=false;
                return false;//说明掉线了
            }
    ...
    function queryJob()
    {
        ...
        $username=$this->spider_info['oj_username'];
        $html=$curl->get("http://acm.hdu.edu.cn/status.php?user=$username",10);
        $table="<tr>" . $this->getSubStr($html,"<tr align=center >","</table>",0);
        $dom=new DOMDocument();
        $dom->loadHTML($table);
        ...
        for($i=0;$i<$trs->length;$i++)
        {
            ...
            $result->wrong_info=$tds->item(2)->textContent;
            $result->time_usage=doubleval($tds->item(4)->textContent);
            $result->ram_usage=doubleval($tds->item(5)->textContent)*1024;
            switch(substr($tds->item(2)->textContent,0,9))
            {
                case 'Accepted':
    ...
    function checkLogin()
    {
        ...
        $curl=new curlRequest();
        $curl->setCookieRaw($this->additionInfo['cookie']);
        $html=$curl->get("http://acm.hdu.edu.cn/status.php?user=".$this->spider_info['oj_username']);
        if(strpos($html,'Register new ID')!==false)
            return false;
        else
            return true;
    ...
    function login()
    {
        ...
        $request=array(
            ...
            "userpass"=>$password,
            "login"=>'Sign In');
        $return=$curl->post("http://acm.hdu.edu.cn/userloginex.php?action=login",$request,10);
